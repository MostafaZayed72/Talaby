...
<template>
  <div class="space-y-8">
    ...
    <div v-if="showMarkDoneDialog" class="fixed inset-0 z-[100] flex items-center justify-center p-6">
      <div class="absolute inset-0" style="background:rgba(2,6,23,0.85);backdrop-filter:blur(4px)" @click="showMarkDoneDialog = false"></div>
      <div
        class="relative p-10 rounded-[2.5rem] shadow-2xl max-w-md w-full text-center space-y-8"
        :style="isDark ? 'background:#0f172a;border:1px solid rgba(255,255,255,0.08);' : 'background:#ffffff;border:1px solid #e2e8f0;'"
      >
        <div class="w-20 h-20 rounded-3xl flex items-center justify-center mx-auto shadow-lg" style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.2);color:#22c55e;">
           <Icon name="ph:check-circle-bold" class="text-4xl" />
        </div>
        <div class="space-y-2">
          <h3 class="text-2xl font-black italic" :style="isDark ? 'color:#f8fafc' : 'color:#0f172a'">{{ $t('Complete Request') }}</h3>
          <p class="font-bold" :style="isDark ? 'color:#94a3b8' : 'color:#475569'">{{ $t('Are you sure you have completed the task and want to mark it as done?') }}</p>
        </div>
        <div class="flex gap-4">
          <button
            class="flex-1 px-6 py-4 rounded-2xl font-black transition-all active:scale-95"
    ...
    <div v-if="showPaymentConfirmDialog" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
      ...
      <div
        class="relative p-8 md:p-12 rounded-[2.5rem] shadow-2xl max-w-md w-full text-center space-y-8"
        :style="isDark ? 'background:#0f172a;border:1px solid rgba(255,255,255,0.08);' : 'background:#ffffff;border:1px solid #e2e8f0;'"
      >
