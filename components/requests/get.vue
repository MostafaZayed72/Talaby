...
<template>
  <div class="p-6 max-w-3xl mx-auto text-center">
    ...
    <div v-if="post" class="space-y-4">
      <h1 class="text-2xl font-bold bg-purple-950 rounded-full p-2 mx-auto text-center text-white">
        {{ post.title }}
      </h1>

      <div v-if="post.imageUrl" class="flex justify-center">
        <img
          :src="post.imageUrl"
          alt="صورة الطلب"
          class="max-w-full max-h-[300px] h-auto rounded-lg shadow-md"
        />
      </div>
