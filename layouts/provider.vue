<template>
  <div :style="{ direction: locale === 'ar' ? 'rtl' : 'ltr' }" class="bg-slate-50 dark:bg-slate-950 min-h-screen font-sans">
    ...
    <!-- Sidebar -->
    <aside 
      class="fixed top-0 bottom-0 z-[60] w-72 bg-indigo-950 border-r border-white/10 transition-transform duration-500 md:translate-x-0 shadow-2xl"
      :class="[
    ...
    >
      <div class="flex flex-col h-full">
        ...
        <div class="p-8 border-b border-white/10 flex items-center gap-4">
          ...
          <span class="text-2xl font-black text-white italic tracking-tighter uppercase">Talaby<span class="text-yellow-400">Pro</span></span>
        </div>
      ...
      <header class="h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 sticky top-0 z-50 px-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          <div class="text-right hidden sm:block">
            <p class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">{{ $t('Service Provider') }}</p>
          ...
          <div class="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white shadow-lg">
             <Icon name="ph:storefront-bold" class="text-2xl" />
          </div>
        </div>
      </header>
...
const providerLinks = [
  ...
  { to: '/dashboard/proposals', label: 'My Proposals', icon: 'ph:file-text-bold' },
  { to: '/', label: 'Visit Website', icon: 'ph:globe-bold' },
];

const pageTitle = computed(() => {
  if (route.path.includes('proposals')) return t('My Proposals');
  return t('Provider Dashboard');
});
...
onMounted(() => {
  loading.value = false;
  // Get email from storage if available
